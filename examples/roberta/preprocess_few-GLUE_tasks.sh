...
if [ "$TASKS" = "ALL" ]
then
  TASKS="QQP MNLI QNLI MRPC RTE STS-B SST-2 CoLA WNLI"
fi
...
do
  ...
  do
    ...
    then
      cp "$TASK_DATA_FOLDER/$SPLIT$K.tsv" "$TASK_DATA_FOLDER/processed/$SPLIT$K.tsv.temp";
    else
      tail -n +2 "$TASK_DATA_FOLDER/$SPLIT$K.tsv" > "$TASK_DATA_FOLDER/processed/$SPLIT$K.tsv.temp";
    fi

    # Remove unformatted lines from train and dev files for QQP dataset.
    if [[ ( "$TASK" = "QQP") && ( "$SPLIT" != "test" ) ]]
    then
      awk -F '\t' -v NUM_FIELDS=6 'NF==NUM_FIELDS{print}{}' "$TASK_DATA_FOLDER/processed/$SPLIT$K.tsv.temp" > "$TASK_DATA_FOLDER/processed/$SPLIT$K.tsv";
    else
      cp "$TASK_DATA_FOLDER/processed/$SPLIT$K.tsv.temp" "$TASK_DATA_FOLDER/processed/$SPLIT$K.tsv";
    fi
    rm "$TASK_DATA_FOLDER/processed/$SPLIT$K.tsv.temp";
  done
  ...
  do
    ...
    do
      ...
      cut -f"$COLUMN_NUMBER" "$TASK_DATA_FOLDER/processed/$SPLIT$K.tsv" > "$TASK_DATA_FOLDER/processed/$SPLIT$K.raw.input$INPUT_TYPE";
    done
    ...
    then
      ...
      then
        cut -f"$DEV_LABEL_COLUMN" "$TASK_DATA_FOLDER/processed/$SPLIT$K.tsv"  > "$TASK_DATA_FOLDER/processed/$SPLIT$K.label";
      else
        cut -f"$LABEL_COLUMN" "$TASK_DATA_FOLDER/processed/$SPLIT$K.tsv" > "$TASK_DATA_FOLDER/processed/$SPLIT$K.label";
      fi
    fi
    ...
    do
      ...
      echo "BPE encoding $SPLIT$K/$LANG"
  ...
  then
    ...
    mkdir -p "$TASK-bin$K/label"
    awk '{print $1 / 5.0 }' "$TASK_DATA_FOLDER/processed/train$K.label" > "$TASK-bin/label/train$K.label"
    awk '{print $1 / 5.0 }' "$TASK_DATA_FOLDER/processed/dev$K.label" > "$TASK-bin/label/valid$K.label"
  fi
done
