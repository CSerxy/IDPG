...
print('---------------------')
print('| bagging: ', (tp / (tp + 0.5 * (fp + fn)) + float(ncorrect)/float(nsamples)) / 2)
...
with open(args.output_file, 'a') as outf:
    outf.write('mrpc bagging' + '| f1 = ' + str(tp / (tp + 0.5 * (fp + fn))) + ' | Accuracy: ' + str(float(ncorrect)/float(nsamples)) + ' | avg: '+ str((tp / (tp + 0.5 * (fp + fn)) + float(ncorrect)/float(nsamples)) / 2) + '\n')
