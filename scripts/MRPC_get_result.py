...
parser.add_argument("-d", "--data_file", default="data/MRPC-dev.tsv", help="the data")
...
roberta = RobertaModel.from_pretrained(
    ...
    data_name_or_path='MRPC-bin'+args.kn,
)
...
with open(args.output_file, 'a') as outf:
    outf.write('mrpc: ' + args.froms + '-' + args.to + '-' + args.number + '-' + args.klr + '| f1 = ' + str(tp / (tp + 0.5 * (fp + fn))) + ' | Accuracy: ' + str(float(ncorrect)/float(nsamples)) + ' | avg: '+ str((tp / (tp + 0.5 * (fp + fn)) + float(ncorrect)/float(nsamples)) / 2) + '\n')
