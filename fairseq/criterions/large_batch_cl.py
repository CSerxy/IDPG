...
from pytorch_metric_learning.losses import GenericPairLoss, CrossBatchMemory
...
@register_criterion('large_batch_cl')
class LBCLLoss(FairseqCriterion):
    """
    ...
    def __init__(self, task, tpu):
        super().__init__(task)
        self.inner_loss = NTXentLoss(temperature=0.10)
        self.loss = None 
        self.offset = 0
    ...
    def forward(self, model, sample, reduce=True):
        ...
        indices = torch.arange(0, batch_size, device=logits1.device)
        labels = torch.cat((indices, indices))
        labels += self.offset 
        self.offset += batch_size

        if self.loss == None:
            self.loss = CrossBatchMemory(loss=self.inner_loss, embedding_size=logits.size(1), memory_size=128)
 
        res = self.loss(logits, labels)
