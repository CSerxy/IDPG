...
@register_task('mlm+mix')
class MIXTask(FairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--cl-mask-prob', default=0.0, type=float,
                            help='probability of replacing a token with mask')
        ...
        parser.add_argument('--switch-token-nums', default=0, type=int,
        ...
        parser.add_argument('--eliminate', default='eli',
                            choices=['eli', 'rep'],
                            help='delete by eliminating or simply replacement consistent [MASK] tokens')
        ...
        parser.add_argument('--subs-prob', default=0.0, type=float,
        ...
        parser.add_argument('--subs-random-token-prob', default=0.1, type=float,
                            help='probability of replacing a token with a random token')
        """parameters for delete spans"""
        parser.add_argument('--del-span-nums', default=0, type=int,
                            help='times of swithing a span of tokens with another span in the sentence')
        parser.add_argument('--del-span-max-prop', default=0.05, type=float,
                            help='max proportion of a switch span takes in whole sentence')
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        src_dataset1, src_dataset2 = MixedDataset.apply_mask(
            dataset,
            self.args.seed,
            self.source_dictionary,
            pad_idx=self.source_dictionary.pad(),
            mask_idx=self.mask_idx,
            cl_mask_prob=self.args.cl_mask_prob,
            leave_unmasked_prob=self.args.leave_unmasked_prob,
            random_token_prob=self.args.random_token_prob,
            freq_weighted_replacement=self.args.freq_weighted_replacement,
            mask_whole_words=mask_whole_words,
            ...
            del_span_nums=self.args.del_span_nums,
            del_span_max_prop=self.args.del_span_max_prop,
            eliminate=(self.args.eliminate == 'eli'), 
            subs=self.subs,
