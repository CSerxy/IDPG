...
class TransformerEncoderWithAdapterLayer(nn.Module):
    """Encoder layer block.
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        ...
        if self.use_adapter:
            self.adapter1_down = Linear(self.embed_dim, bottleneck_dim)
            self.adapter1_up = Linear(bottleneck_dim, self.embed_dim)

            self.adapter2_down = Linear(self.embed_dim, bottleneck_dim)
            self.adapter2_up = Linear(bottleneck_dim, self.embed_dim)
    ...
    def forward(self, x, encoder_padding_mask, attn_mask: Optional[Tensor] = None):
        ...
        if self.use_adapter:
            residual_adapter = x
            x = self.adapter1_down(x)
            x = self.activation_fn(x)
            x = self.adapter1_up(x)
        ...
        if self.use_adapter:
            residual_adapter = x
            x = self.adapter2_down(x)
            x = self.activation_fn(x)
            x = self.adapter2_up(x)
            x = residual_adapter + x
...
class TransformerDecoderWithAdapterLayer(nn.Module):
    """Decoder layer block.
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(
        ...
        if self.use_adapter:
            ...
            self.adapter3_down = Linear(self.embed_dim, bottleneck_dim)
            self.adapter3_up = Linear(bottleneck_dim, self.embed_dim)
    ...
    def forward(
        ...
        if self.use_adapter:
            residual_adapter = x
            x = self.adapter3_down(x)
            x = self.activation_fn(x)
            x = self.adapter3_up(x)
            x = residual_adapter + x
...
def Linear(in_features, out_features, bias=True):
    ...
    # logger.info('initializing linear: {}'.format(m.weight))
    if bias:
